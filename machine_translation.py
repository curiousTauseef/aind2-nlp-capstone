...
from keras.layers import GRU, Input, Dense, TimeDistributed, Activation, RepeatVector, Bidirectional, Dropout, BatchNormalization, regularizers
...
def tokenize(x):
    ...
    """
    # TODO: Implement
    x_tokenizer = Tokenizer(num_words=len(french_words_counter),char_level=False)
    x_tokenizer.fit_on_texts(x)
    tokenized_x_data = x_tokenizer.texts_to_sequences(x)
    return tokenized_x_data, x_tokenizer
...
def simple_model(input_shape, output_sequence_length, english_vocab_size, french_vocab_size):
    ...
    model = Sequential()
    model.add(LSTM(output_sequence_length,
...
from keras.layers import GRU,LSTM, Input, Dense, TimeDistributed, Embedding
from keras.models import Model, Sequential
from keras.layers import Activation, RepeatVector
from keras.optimizers import Adam
from keras.losses import sparse_categorical_crossentropy
...
def encdec_model(input_shape, output_sequence_length, english_vocab_size, french_vocab_size):
    ...
    model = Sequential()
    # encoder input model
    model.add(LSTM(units = output_sequence_length,
                   input_shape = input_shape[1:],
